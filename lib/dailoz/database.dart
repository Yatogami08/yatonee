...
class DatabaseHelper {
  ...
  Future<Database> initDatabase() async {
    String path = join(await getDatabasesPath(), 'yatomihihi.db');
    return openDatabase(
      path,
      version: 1,
...
// Tạo bảng miennam
 ...
 id INTEGER PRIMARY KEY AUTOINCREMENT,
  ...
  Future<List<String>> getYatoValues() async {
    Database db = await database;

    List<Map<String, dynamic>> results = await db.query('nhiemvuhn', columns: ['yato_4', 'yato_5'], where: 'ca_dangki = ?', whereArgs: ['5h']);

    List<String> yatoValues = [];

    for (Map<String, dynamic> result in results) {
      yatoValues.add(result['yato_4']);
      yatoValues.add(result['yato_5']);
    }
  ...
  Future<String> sumTime(List<Map<String, dynamic>> rows) async {
    int totalMinutes = 0;

    for (var row in rows) {
      String timeString = row['total_thoigian_hd'];
      List<String> parts = timeString.split(' ');
      for (var part in parts) {
        if (part.contains('giờ')) {
          totalMinutes += int.parse(part.split('giờ')[0].trim()) * 60;
        } else if (part.contains('phút')) {
          totalMinutes += int.parse(part.split('phút')[0].trim());
        }
      }
    }
    ...
    return '$days ngày $remainingHours giờ $remainingMinutes phút';
  }
...
COALESCE(SUM(SUBSTR(REPLACE(thoigian_hd, ' giờ ', '.'), 1, INSTR(REPLACE(thoigian_hd, ' giờ ', '.'), '.') - 1)) + 
               SUM(SUBSTR(REPLACE(thoigian_hd, ' giờ ', '.'), INSTR(REPLACE(thoigian_hd, ' giờ ', '.'), '.') + 1, INSTR(thoigian_hd, 'phút') - INSTR(REPLACE(thoigian_hd, ' giờ ', '.'), '.') - LENGTH('giờ')) / 60.0), 0) as total_thoigian_hd,
  ...
  Future<List<Map<String, dynamic>>> getNhiemvuData() async {
    ...
    String nhiemvu = '';
    if (getCalkhuvuc == 'hanoi') {
      nhiemvu = 'nhiemvuhn'; // Tên bảng tương ứng với Hà Nội
    } else if (getCalkhuvuc == 'miennam') {
      nhiemvu = 'nhiemvuhcm'; // Tên bảng tương ứng với Miền Nam
    } else if (getCalkhuvuc == 'danang') {
      nhiemvu = 'nhiemvudanang'; // Tên bảng tương ứng với Đà Nẵng
    }
